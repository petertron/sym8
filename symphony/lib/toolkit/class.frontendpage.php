...
class FrontendPage extends XSLTPage
{
    ...
    private function __buildPage()
    {
        ...
        $this->_param = array(
            ...
            'this-week-number' => $date->format('W'),
    ...
    public function resolvePage($page = null)
    {
        ...
        if (isset($page_extra_bits)) {
            ...
            for ($i = 0, $ii = count($page_extra_bits); $i < $ii; $i++) {
                // added by tiloschroeder.de
                // if a subpage doesn't exist, $url_params[$i] leads to an undefined array key 0
                // so check first, if $url_params isn't empty, otherwise return false (404 page)
                if ( empty($url_params) ) {
                    return false;
                } else {
