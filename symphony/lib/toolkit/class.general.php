...
class General
{
     ...
     */
    public static function createHandle($string, $max_length = 200, $delim = '-', $uriencode = false, $additional_rule_set = null)
    {
        ...
        // Remove possible multiple $delim
        // by tiloschroeder
        $string = str_replace('/' . preg_quote($delim) . '+/', $delim, $string);
    ...
    /**
     * Checks whether a Symphony handle is already in use and increments it.
     * @used by field.input.php
     *
     *  Example:
     *  hello-world   → already in use
     *  hello-world-1 → already in use
     *  hello-world-2 → not in use → returned
     *
     * @since Symphony 2.83.0
     * @author tiloschroeder
     * @param string $baseHandle
     *  Handle to be checked
     * @param integer $field_id
     * Field ID of the input field
     * @param integer $entry_id
     * Current entry ID (used to exclude itself when editing)
     * @return string
     *  Unique handle (possibly incremented)
     */
    public static function incrementHandle($baseHandle, $field_id, $entry_id = null)
    {
        $db = Symphony::Database();
        $i = 1;
        $handle = $baseHandle;

        $escaped_handle = $db->cleanValue($handle);

        while (true) {
            $query = "SELECT `id` FROM `tbl_entries_data_{$field_id}` WHERE `handle` = '{$escaped_handle}'";
            if (is_numeric($entry_id)) {
                $query .= " AND `entry_id` != " . intval($entry_id);
            }

            $existing = $db->fetchVar('id', 0, $query);
    ...
    /**
     * Copy a file from the input path to the destination path.
     * If the file already exists or the creation of the directory fails then this will return false.
     *
     * This method is primarily intended for binary files (e.g., images, icons),
     * where direct copying is preferred. For text files requiring content modifications,
     * use `General::writeFile()` instead.
     *
     ...
     * @param boolean $log
     *  write to Symphony log
     * @return boolean
     *  true if the file is successfully copied to the destination path, otherwise false.
     */
    public static function copyFile(string $source, string $destination, bool $log = true): bool
    {
        // Check if source exists
        if (!is_file($source)) {
            if ($log) {
                Symphony::Log()->pushToLog("Source file does not exist: {$source}", E_NOTICE, true, true);
            }
            return false;
        }
        ...
        if (!is_dir($dir)) {
            if (!General::realiseDirectory($dir)) {
                if ($log) {
                    Symphony::Log()->pushToLog("Could not create destination directory: {$dir}", E_NOTICE, true, true);
                }
                return false;
            }
        }
        ...
        if (!@copy($source, $destination)) {
            if ($log) {
                Symphony::Log()->pushToLog("Failed to copy {$source} to {$destination}", E_NOTICE, true, true);
            }
            return false;
        }
